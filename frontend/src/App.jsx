  ...
  React.useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        console.error("Failed to fetch books for AI Assistant", err);
      }
    };
    fetchBooks();
  }, []);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className={`flex flex-col md:flex-row justify-between items-center gap-10 ${i18n.language === 'ar' ? 'rtl' : ''}`}>
              <div className="flex flex-col items-center md:items-start space-y-4">
                ...
                <p className="text-sm font-medium max-w-xs text-center md:text-left opacity-80">{t("footer_text") || "Your next favorite book is just a click away. Discover millions of stories instantly."}</p>
              </div>
            ...
            <div className="border-t border-gray-100 dark:border-gray-900 mt-12 pt-8 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest opacity-40">&copy; {new Date().getFullYear()} BookVerse Digital. {t("all_rights_reserved") || "All rights reserved."}</p>
            </div>
          </div>
        </footer>
      )}
